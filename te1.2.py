...
class Player():
	...
	def placePiece(self, piece, space, board_instance): 
		if not board_instance.checkValidPlacement(piece,space):
			...
			board_instance.kingdoms = board_instance.updateKingdoms(space, self)
	...
	def placeMonument(self, candidates, color, board_instance, hastr=False):
		...
		while not a1 or a1.lower() not in 'yn':
			a1 = input(f"A {color} monument can be placed on {' or '.join(repr(candidates))}. Proceed? [y/n]\n")
		if a1.lower() == 'n':
			return False
...
class Board():
	...
	def updateKingdoms(self, just_placed, active_player):
		...
		if self.coords[just_placed].ptype == 'tile':
			if self.checkHowManyKingdoms(just_placed) == 2: 
	...
	def gameSetup(self):
		...
		while not a2 or a2 not in '234':
			a2 = input('How many players [2 to 4]?\n')
	...
	def endGame(self):
		...
		for attempt in range(4):
			...
			if len(victor) != 1:
				input(f"Players {' and '.join(victor)} are tied after {attempt+1} attempts, with {minpoints[victor[0]][attempt][1]} points!\nBreaking tie...")
				continue				
			else:
				nth = ['st','nd','rd','th']
				input(f"And the victory goes to {victor[0]}! His {attempt+1}{nth[attempt]} lowest color score is {minpoints[victor[0]][attempt][1]}.")
				sys.exit()
				return
		print(f"It's a tie! {' and '.join(victor)} have {minpoints[victor[0]][attempt][1]} points!")
		input('')
...
if __name__ == '__main__':
	...
	while not a1 or a1.lower() not in 'sa':
		a1 = input('Do you want to play on the [s]tandard or [a]dvanced board?\n')
	if a1.lower() == 'a':
